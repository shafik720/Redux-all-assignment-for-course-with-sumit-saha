...
const SingleVideoList = ({video}) => {
    const{url, title, views, duration, description, createdAt, id} = video; 

    const dispatch = useDispatch();
    
    const handleClick = () => {
        dispatch(changeVideo({
            url, title, views, duration, description, createdAt, id
        }));
    }
    
    return (
        <div draggable onClick={handleClick} className="w-full flex flex-row gap-2 cursor-pointer hover:bg-slate-900 p-2 py-3">
            {/* <!-- Thumbnail --> */}

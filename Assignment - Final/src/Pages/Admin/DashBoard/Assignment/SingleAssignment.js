...
const SingleAssignment = ({ assignment }) => {
    ...
    const convertedTitle = title?.split(" ").slice(0, 10).join(" ");
    const convertedVideoTitle = video_title?.split(" ").slice(0, 10).join(" ");

    // --- deleting function
    const [deleteAssignment, { data, isLoading, isError, error, isSuccess }] = useDeleteAssignmentMutation();
    const handleDelete = () => {
    ...
    return (
        <tr>
            <td className="table-td">{convertedTitle}</td>
            <td className="table-td">{convertedVideoTitle}</td>
            ...
            <td className="table-td flex gap-x-2">
                ...
                {/* --- edit button --- */}
                <Link to={`/admin/editAssignment/${id}`}>
                    <svg fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor"
